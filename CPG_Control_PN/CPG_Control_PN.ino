...
double rate_offset = 1; // Adaption rate for offset, it's a new variable for us
...
double offset_dif(int osc_num){
  double X = (double) osc[osc_num].targetOffset;
  double x = (double) osc[osc_num].offset;

  // c is defined as the adaption rate of the offset as well, so we're just using it
  return (double) rate_offset*(X-x);
}
